...
- Added a separate Fabric build for Minecraft `26.1`, `26.1.1`, and `26.1.2`
- Added `/optimizer presets` to show the available presets and what each one is meant for
- Added `/optimizer preview recommended` so players can inspect the recommended preset before applying it
- Fixed graphics option detection on the `26.1` line so recommended/manual presets apply instead of failing safely
- Updated menu and command compatibility for the newer client API
- Improved optimizer menu wording so inactive preset state is shown as `Selected: None` instead of looking like the `Restore` action
- Kept the protected settings untouched: Max FPS, VSync, FOV, GUI Scale, Brightness, and resolution/fullscreen
