...
public final class OptimizerCommands {
        ...
        dispatcher.register(
            ClientCommands.literal("optimizer")
                ...
                .then(ClientCommands.literal("preview")
                    ...
                    .then(buildPreviewLiteral("ultimate_performance", PresetProfile.ULTIMATE_PERFORMANCE, presetApplier))
                    .then(buildPreviewLiteral("performance", PresetProfile.PERFORMANCE, presetApplier))
                    .then(buildPreviewLiteral("balanced", PresetProfile.BALANCED, presetApplier))
                    .then(buildPreviewLiteral("quality", PresetProfile.QUALITY, presetApplier))
                    .then(buildPreviewLiteral("restore", PresetProfile.OFF, presetApplier)))
    ...
    private static com.mojang.brigadier.builder.LiteralArgumentBuilder<FabricClientCommandSource> buildPreviewLiteral(
        ...
        return ClientCommands.literal(literal)
            .executes(context -> feedbackLines(
                context.getSource(),
                presetApplier.previewPresetLines(context.getSource().getClient(), preset)
            ));
    }
    ...
    private static int showHelp(FabricClientCommandSource source) {
        return feedbackLines(
            ...
            "- /optimizer presets -> explains each preset and marks recommended/selected",
            "- /optimizer recommend -> applies the recommended preset",
            "- /optimizer preview recommended -> preview the detected recommendation",
            "- /optimizer preview <preset> -> show what a preset would change",
            ...
            "- /optimizer status -> show selected preset, recommendation, restore, and safety info",
