package com.secondmod.autotunefps.client;

import com.google.gson.Gson;
import com.google.gson.GsonBuilder;
import com.google.gson.JsonParseException;
import com.secondmod.autotunefps.AutoTuneFps;
import net.fabricmc.loader.api.FabricLoader;

import java.io.IOException;
import java.io.Reader;
import java.io.Writer;
import java.nio.file.Files;
import java.nio.file.Path;

public final class AutoTuneRuntimeState {
    private static final Gson GSON = new GsonBuilder().setPrettyPrinting().create();
    private static final Path STATE_PATH = FabricLoader.getInstance()
        .getConfigDir()
        .resolve(AutoTuneFps.MOD_ID + "_state.json");

    public boolean pendingApply = false;
    public String pendingPreset = null;
    public String pendingSource = null;
    public boolean recoveryModeActive = false;
    public boolean recoveryNoticePending = false;
    public String lastInterruptedPreset = null;
    public String lastInterruptedSource = null;
    public RestoreSnapshot restoreSnapshot = null;
    public String lastSmartLabel = null;
    public PresetProfile lastSmartBasePreset = null;
    public SmartTargetSnapshot lastSmartTarget = null;

    public static AutoTuneRuntimeState load() {
        if (Files.notExists(STATE_PATH)) {
            return new AutoTuneRuntimeState();
        }

        try (Reader reader = Files.newBufferedReader(STATE_PATH)) {
            AutoTuneRuntimeState state = GSON.fromJson(reader, AutoTuneRuntimeState.class);
            if (state == null) {
                throw new JsonParseException("Runtime state file was empty");
            }

            state.sanitize();
            return state;
        } catch (IOException | JsonParseException exception) {
            AutoTuneFps.LOGGER.warn("Failed to load runtime state, starting clean", exception);
            return new AutoTuneRuntimeState();
        }
    }

    public void save() {
        sanitize();

        try {
            Files.createDirectories(STATE_PATH.getParent());
            try (Writer writer = Files.newBufferedWriter(STATE_PATH)) {
                GSON.toJson(this, writer);
            }
        } catch (IOException exception) {
            AutoTuneFps.LOGGER.warn("Failed to save runtime state", exception);
        }
    }

    public void markPendingApply(String presetName, String source) {
        pendingApply = true;
        pendingPreset = normalizePreset(presetName);
        pendingSource = normalizeSource(source);
        save();
    }

    public void clearPendingApply() {
        if (!pendingApply && pendingPreset == null && pendingSource == null) {
            return;
        }

        pendingApply = false;
        pendingPreset = null;
        pendingSource = null;
        save();
    }

    public String recoverIfInterrupted(AutoTuneConfig config) {
        if (!pendingApply) {
            return null;
        }

        lastInterruptedPreset = pendingPreset;
        lastInterruptedSource = pendingSource;
        recoveryModeActive = true;
        recoveryNoticePending = true;
        pendingApply = false;
        pendingPreset = null;
        pendingSource = null;

        if (config.autoApplyMode != AutoApplyMode.OFF || config.applyPresetOnWorldJoin) {
            config.autoApplyMode = AutoApplyMode.OFF;
            config.applyPresetOnWorldJoin = false;
            config.save();
        }

        save();
        return describeInterruptedApply();
    }

    public String consumeRecoveryNotice() {
        if (!recoveryNoticePending) {
            return null;
        }

        recoveryNoticePending = false;
        save();
        return "AutoTune paused auto-apply after an interrupted preset apply. Check /optimizer status.";
    }

    public String describeRecoveryState() {
        if (!recoveryModeActive) {
            return null;
        }

        StringBuilder builder = new StringBuilder("auto-apply paused after an interrupted apply");
        String interruptedApply = describeInterruptedApply();
        if (interruptedApply != null) {
            builder.append(" (").append(interruptedApply).append(")");
        }
        return builder.toString();
    }

    public boolean clearRecoveryMode() {
        if (!recoveryModeActive && !recoveryNoticePending && lastInterruptedPreset == null && lastInterruptedSource == null) {
            return false;
        }

        recoveryModeActive = false;
        recoveryNoticePending = false;
        lastInterruptedPreset = null;
        lastInterruptedSource = null;
        save();
        return true;
    }

    public void saveRestoreSnapshot(RestoreSnapshot snapshot) {
        restoreSnapshot = snapshot;
        sanitize();
        save();
    }

    public RestoreSnapshot restoreSnapshot() {
        return restoreSnapshot;
    }

    public boolean hasRestoreSnapshot() {
        return restoreSnapshot != null;
    }

    public boolean clearRestoreSnapshot() {
        if (restoreSnapshot == null) {
            return false;
        }

        restoreSnapshot = null;
        save();
        return true;
    }

    public String describeRestoreState() {
        return hasRestoreSnapshot() ? "Available" : "Unavailable";
    }

    public void rememberSmartPreset(SmartPresetRecommendation recommendation) {
        lastSmartLabel = normalizePreset(recommendation.basePreset().displayName());
        lastSmartBasePreset = recommendation.basePreset();
        lastSmartTarget = SmartTargetSnapshot.from(recommendation.target());
        save();
    }

    public void clearSmartPreset() {
        if (lastSmartLabel == null && lastSmartBasePreset == null && lastSmartTarget == null) {
            return;
        }

        lastSmartLabel = null;
        lastSmartBasePreset = null;
        lastSmartTarget = null;
        save();
    }

    public PresetTarget lastSmartTarget() {
        return lastSmartTarget == null ? null : lastSmartTarget.toPresetTarget();
    }

    public String smartModeLabel() {
        if (lastSmartLabel == null) {
            return null;
        }
        if (lastSmartBasePreset != null) {
            return "Recommended " + lastSmartBasePreset.displayName();
        }
        return "Recommended " + trimSmartSuffix(lastSmartLabel);
    }

    private String describeInterruptedApply() {
        String source = displayValue(lastInterruptedSource);
        String preset = displayValue(lastInterruptedPreset);
        if (source == null && preset == null) {
            return null;
        }
        if (source == null) {
            return "last preset: " + preset;
        }
        if (preset == null) {
            return "source: " + source;
        }
        return "source: " + source + ", preset: " + preset;
    }

    private static String displayValue(String value) {
        if (value == null || value.isBlank()) {
            return null;
        }
        return value;
    }

    private static String normalizePreset(String presetName) {
        if (presetName == null || presetName.isBlank()) {
            return null;
        }
        return presetName;
    }

    private static String trimSmartSuffix(String presetName) {
        if (presetName.endsWith("+") || presetName.endsWith("-")) {
            return presetName.substring(0, presetName.length() - 1);
        }
        return presetName;
    }

    private static String normalizeSource(String source) {
        if (source == null || source.isBlank()) {
            return null;
        }
        return source;
    }

    private void sanitize() {
        pendingPreset = normalizePreset(pendingPreset);
        pendingSource = normalizeSource(pendingSource);
        lastInterruptedPreset = normalizePreset(lastInterruptedPreset);
        lastInterruptedSource = normalizeSource(lastInterruptedSource);
        if (restoreSnapshot != null) {
            restoreSnapshot.sanitize();
            if (restoreSnapshot.isEmpty()) {
                restoreSnapshot = null;
            }
        }
        lastSmartLabel = normalizePreset(lastSmartLabel);
        if (lastSmartBasePreset == null || !lastSmartBasePreset.isEnabled()) {
            lastSmartBasePreset = null;
        }
        if (lastSmartTarget != null) {
            lastSmartTarget.sanitize();
            if (lastSmartTarget.isEmpty()) {
                lastSmartTarget = null;
            }
        }
        if (!recoveryModeActive && !recoveryNoticePending && lastInterruptedPreset == null && lastInterruptedSource == null) {
            return;
        }
    }

    public static final class RestoreSnapshot {
        public String graphicsMode = null;
        public String clouds = null;
        public String particles = null;
        public String chunkUpdatePriority = null;
        public Integer renderDistance = null;
        public Integer simulationDistance = null;
        public Double entityDistanceScaling = null;
        public Integer biomeBlend = null;
        public Boolean entityShadows = null;

        private void sanitize() {
            graphicsMode = normalizeValue(graphicsMode);
            clouds = normalizeValue(clouds);
            particles = normalizeValue(particles);
            chunkUpdatePriority = normalizeValue(chunkUpdatePriority);
        }

        private boolean isEmpty() {
            return graphicsMode == null
                && clouds == null
                && particles == null
                && chunkUpdatePriority == null
                && renderDistance == null
                && simulationDistance == null
                && entityDistanceScaling == null
                && biomeBlend == null
                && entityShadows == null;
        }

        private static String normalizeValue(String value) {
            if (value == null || value.isBlank()) {
                return null;
            }
            return value;
        }
    }

    public static final class SmartTargetSnapshot {
        public String graphicsMode = null;
        public String clouds = null;
        public String particles = null;
        public String chunkUpdatePriority = null;
        public Integer renderDistance = null;
        public Integer simulationDistance = null;
        public Double entityDistanceScaling = null;
        public Integer biomeBlend = null;
        public Boolean entityShadows = null;

        public static SmartTargetSnapshot from(PresetTarget target) {
            SmartTargetSnapshot snapshot = new SmartTargetSnapshot();
            snapshot.graphicsMode = target.graphicsMode();
            snapshot.clouds = target.clouds();
            snapshot.particles = target.particles();
            snapshot.chunkUpdatePriority = target.chunkUpdatePriority();
            snapshot.renderDistance = target.renderDistance();
            snapshot.simulationDistance = target.simulationDistance();
            snapshot.entityDistanceScaling = target.entityDistanceScaling();
            snapshot.biomeBlend = target.biomeBlend();
            snapshot.entityShadows = target.entityShadows();
            return snapshot;
        }

        public PresetTarget toPresetTarget() {
            return new PresetTarget(
                graphicsMode,
                clouds,
                particles,
                renderDistance == null ? 8 : renderDistance,
                simulationDistance == null ? 5 : simulationDistance,
                entityDistanceScaling == null ? 1.0D : entityDistanceScaling,
                biomeBlend == null ? 0 : biomeBlend,
                entityShadows == null || entityShadows,
                chunkUpdatePriority
            );
        }

        private void sanitize() {
            graphicsMode = RestoreSnapshot.normalizeValue(graphicsMode);
            clouds = RestoreSnapshot.normalizeValue(clouds);
            particles = RestoreSnapshot.normalizeValue(particles);
            chunkUpdatePriority = RestoreSnapshot.normalizeValue(chunkUpdatePriority);
        }

        private boolean isEmpty() {
            return graphicsMode == null
                && clouds == null
                && particles == null
                && chunkUpdatePriority == null
                && renderDistance == null
                && simulationDistance == null
                && entityDistanceScaling == null
                && biomeBlend == null
                && entityShadows == null;
        }
    }
}
