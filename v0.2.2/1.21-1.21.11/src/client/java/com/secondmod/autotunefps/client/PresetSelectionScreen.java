...
public final class PresetSelectionScreen extends Screen {
    ...
    private void addFixedButton(String label, int x, int y, int width, int height, net.minecraft.client.gui.components.Button.OnPress onPress) {
    ...
    public void render(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTick) {
        ...
        for (GlassButton button : fixedButtons) {
            button.widget().render(guiGraphics, mouseX, mouseY, partialTick);
            button.renderGlass(guiGraphics);
        }

        guiGraphics.enableScissor(0, CONTENT_TOP, this.width, contentBottom());
        drawOptimizationSettingsHeading(guiGraphics);
        for (ScrollEntry entry : scrollEntries) {
            entry.button().widget().render(guiGraphics, mouseX, mouseY, partialTick);
            entry.button().renderGlass(guiGraphics);
        }
        guiGraphics.disableScissor();
        drawScrollbar(guiGraphics);
    }

    private void drawFixedChrome(GuiGraphics guiGraphics) {
        ...
        guiGraphics.drawCenteredString(this.font, "AutoTune Presets", this.width / 2, 13, 0xFFFFFFFF);
        guiGraphics.drawCenteredString(
            this.font,
            "Recommended preset: " + formatSmartRecommendationLabel(),
            this.width / 2,
            29,
            0xFFD2D2D3
        );
    ...
    private void drawOptimizationSettingsHeading(GuiGraphics guiGraphics) {
        ...
        guiGraphics.drawCenteredString(this.font, heading, this.width / 2, y, 0xFFE1E1E2);
    }
    ...
    private void onAutoApplyButtonPressed(net.minecraft.client.gui.components.Button button) {
        toggleAutoApply();
    }

    private void onOptimizationEngineButtonPressed(net.minecraft.client.gui.components.Button button) {
        updateOptimizationEngineMode();
    }

    private void onParticleReductionButtonPressed(net.minecraft.client.gui.components.Button button) {
        toggleParticleReduction();
    }

    private void onDoneButtonPressed(net.minecraft.client.gui.components.Button button) {
        onClose();
    }

    private void onSmartApplyButtonPressed(net.minecraft.client.gui.components.Button button) {
        runMenuAction(
    ...
    private void onSmartPreviewButtonPressed(net.minecraft.client.gui.components.Button button) {
        ...
        for (String line : lines) {
            this.minecraft.player.displayClientMessage(Component.literal(line), false);
        }
    }

    private void onSmartRescanButtonPressed(net.minecraft.client.gui.components.Button button) {
        if (this.minecraft != null) {
