...
public final class PresetApplier {
    ...
    public String currentModeLabel(Minecraft client) {
        ...
        if (client == null) {
            return "Saved preset selected (live settings unavailable)";
        }
    ...
    public String[] describeStatusLines(Minecraft client) {
        ...
        lines.add("- Active mode: " + currentModeLabel(client));
        if (config.selectedPreset.isEnabled()) {
            lines.add("- Saved preset: " + config.selectedPreset.displayName());
        }
        lines.add("- Smart Recommendation: " + smartRecommendation.basePreset().displayName());
        ...
        if (isGraphicsModeUnavailable(client)) {
            lines.add("- Graphics mode: unavailable on this Minecraft version (skipped)");
        }
    ...
    public String[] previewPresetLines(Minecraft client, PresetProfile preset) {
        ...
        if (!preset.isEnabled()) {
            ...
            lines.add("- Would restore your original settings from before AutoTune FPS changed them.");
            lines.add("- Restore snapshot: " + runtimeState.describeRestoreState());
        ...
        PresetTarget currentTarget = client == null ? null : captureCurrentPresetTarget(client);
        lines.add("AutoTune preset preview: " + preset.displayName());
        if (currentTarget == null) {
            ...
            if (countChangedSettings(currentTarget, target) == 0) {
                lines.add("- No setting changes needed.");
            }
        }
        lines.add("- This preview does not change any settings.");
        return lines.toArray(String[]::new);
    ...
    public String[] previewSmartRecommendedPresetLines(Minecraft client) {
        ...
        PresetTarget currentTarget = client == null ? null : captureCurrentPresetTarget(client);
        List<String> lines = new ArrayList<>();
        lines.add("Smart Recommendation Preview: " + recommendation.basePreset().displayName());
        if (currentTarget == null) {
            lines.add("- Current settings unavailable.");
        } else {
    ...
    public String[] describePresetGuideLines(Minecraft client) {
        ...
        lines.add("- Hardware tier: " + displayTier(profile.tier()));
        lines.add("- Smart Recommendation: " + smartRecommendation(client).basePreset().displayName());
    ...
    private String applySmartRecommended(Minecraft client, String source) {
        ...
        return describeAppliedPreset(
            "Applied Smart Recommendation: " + recommendation.basePreset().displayName() + ".",
            notes,
            clearedRecoveryMode
        );
    }
    ...
    private void showRecoveryNoticeIfNeeded(Minecraft client) {
        ...
        client.player.displayClientMessage(Component.literal(recoveryNotice), false);
    }
