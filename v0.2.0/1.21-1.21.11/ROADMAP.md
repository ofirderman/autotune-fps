...
Internal development notes only. This file records ideas and direction; it is not public release copy or a feature promise.
...
Its goal is to be the easy optimization layer that helps regular players get better settings fast and with minimal effort.
...
- predictable automatic optimization
...
- a recovery layer for users who do not want to tweak every setting manually
...
- protected settings application with runtime fallbacks
- hardware-level-aware recommendations
...
Only apply low-risk settings in v0.1, such as:
- particles
- clouds
- graphics-related toggles
- chunk update priority where appropriate
- entity-related distance options where compatible
...
- it works across the supported `1.21-1.21.11` patch range
- the project becomes a stable base for later work
...
- smarter preset recommendations exposed in the mod menu
- better config editing
- clearer explanations for each preset
- clear revert to previous settings
...
### Scope
- recovery mode after bad launches
...
- make the mod feel clear and friendly
