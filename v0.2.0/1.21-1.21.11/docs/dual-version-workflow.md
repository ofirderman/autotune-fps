...
- `26.1-26.1.2` line: a separate port workspace under `project/ports/`

## Canonical Layout

- `build/libs/AutoTune FPS vX.Y.Z.jar`
  Main `1.21-1.21.11` release jar from the repo root
- `build/libs/AutoTune FPS vX.Y.Z-26.1-26.1.2.jar`
  Convenience copy of the separate `26.1-26.1.2` release jar in the main repo libs folder
- `project/ports/vX.Y.Z-26.1-26.1.2/build/libs/AutoTune FPS vX.Y.Z-26.1-26.1.2.jar`
  Separate `26.1-26.1.2` release jar

## Source Of Truth

- The repo root stays the active main development line
- A `26.1-26.1.2` port is created as a separate workspace
...
5. Build and verify the `26.1-26.1.2` jar there
6. Keep both jars:
   - `build/libs/AutoTune FPS vX.Y.Z.jar`
   - `build/libs/AutoTune FPS vX.Y.Z-26.1-26.1.2.jar`
   - `project/ports/vX.Y.Z-26.1-26.1.2/build/libs/AutoTune FPS vX.Y.Z-26.1-26.1.2.jar`
...
- Building a `26.1-26.1.2` port also copies its release jar into the main repo `build/libs` folder for easier access
- The helper script does not publish anything and does not touch gameplay code
- Runtime testing is still required for the new `26.1-26.1.2` jar after the workspace is created
