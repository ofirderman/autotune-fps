...
- active port: Minecraft `26.1` through `26.1.2`
- companion 0.2.0 port: Minecraft `1.21` through `1.21.11`
...
Every public preset is adaptive. AutoTune resolves the selected preset against the detected hardware tier and compatibility signals before applying settings.
...
`Smart Recommendation` is a separate action, not a fourth visual preset.
...
It recommends `Performance`, `Balanced`, or `Quality` from detected hardware and compatibility signals:
...
Smart Recommendation never automatically chooses `ultimate_performance`.
...
- `autotune_fps.json` stores the selected preset, the auto-apply mode, and compatibility flags
- `autotune_fps_state.json` stores the original pre-AutoTune restore baseline, the last Smart target, and temporary recovery state for interrupted preset applies

Auto-apply modes:

- `Off` does nothing on world/server join
- `Selected preset` applies the saved manual preset on join
- `Smart recommendation` computes and applies the current Smart Recommendation once on join
