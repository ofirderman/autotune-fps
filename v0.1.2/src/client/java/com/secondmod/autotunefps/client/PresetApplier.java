...
public final class PresetApplier {
    ...
    public String[] describeStatusLines(Minecraft client) {
        ...
        lines.add("- Preset: " + formatPresetWithTier(config.selectedPreset, profile.tier()));
        lines.add("- Recommend: " + formatPresetWithTier(profile.recommendedPreset(), profile.tier()));
    ...
    private String restoreSavedSettingsAndRemember(Minecraft client) {
        ...
        ApplyRuntimeNotes notes = restoreSavedSettings(client, restoreSnapshot);
        client.options.save();
    ...
    private static AutoTuneRuntimeState.RestoreSnapshot captureRestoreSnapshot(Options options) {
        ...
        snapshot.graphicsMode = currentEnumName(options.graphicsMode());
    ...
    private static ApplyRuntimeNotes restoreSavedSettings(Minecraft client, AutoTuneRuntimeState.RestoreSnapshot snapshot) {
        ...
        if (snapshot.graphicsMode != null) {
            graphicsModeOutcome = setGraphicsModeOption(client, options.graphicsMode(), snapshot.graphicsMode);
        }
